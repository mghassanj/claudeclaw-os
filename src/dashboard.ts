...
import { runMainTurnQueued, parseBridgeAttachments, INBOUND_TYPES, type MainTurnMeta } from './main-turn.js';
...
export function buildDashboardApp(botApi?: Api<RawApi>): Hono {
  ...
  app.post('/api/agent/main-turn', async (c) => {
    ...
    const inboundType = INBOUND_TYPES.includes(body?.inboundType)
    ...
    const attachments = parseBridgeAttachments(body?.attachments);
    const meta: MainTurnMeta = { channel: str(body?.channel), lang: str(body?.lang), inboundType, image, attachments };
    if (!text && !image && !attachments?.length) return c.json({ error: 'text required' }, 400);
    try {
